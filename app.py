...
if GEMINI_API_KEY:
    try:
        ...
        model = genai.GenerativeModel("gemini-3.1-flash-lit")
    except Exception:
        pass
...
- 답변은 간결하게 유지"""
    ...
    SHORTCUTS = [
        ("좌석 많은 도서관 TOP 3",  "좌석이 많고 접근성 좋은 도서관 TOP3를 이유와 함께 추천해줘"),
        ("장서 많은 공공도서관",     "공공도서관 중 장서가 가장 많은 곳 TOP3를 알려줘"),
        ("주말에도 여는 도서관",     "토요일에 운영하는 도서관 목록을 구/군별로 정리해줘"),
        ("어린이 전용 도서관",       "어린이도서관 또는 아동 관련 도서관을 추천해줘"),
        ("오늘 방문하기 좋은 곳",    "평일 기준 늦게까지 운영하는 도서관은 어디야?"),
        ("구별 도서관 현황 요약",     "부산 각 구/군별로 도서관이 몇 개씩 있는지 간단히 정리해줘"),
    ]

    col_count = 3
    rows = [SHORTCUTS[i:i+col_count] for i in range(0, len(SHORTCUTS), col_count)]
    for row_items in rows:
        cols = st.columns(len(row_items))
        for col, (label, prompt) in zip(cols, row_items):
            with col:
                if st.button(label, use_container_width=True):
                    st.session_state["shortcut"] = prompt
